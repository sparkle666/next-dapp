...
// function
  ...
  const getUsdc = async () => {
    const usdcContract = await new ethers.Contract(usdc.address, usdc.abi, signer)
      const usdbal = await usdcContract.name()
      const sym = await usdcContract.symbol();
      const bal = await provider.getBalance("0x1D3E0725bD6dAf542C780AeDF28553B399556697")
      ...
      console.log({
        usdcContract, sym, name, balEth, hash
     })
  }
  return (
